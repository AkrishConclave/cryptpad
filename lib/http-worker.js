...
// Trust the reverse proxy to get correct IP addresses for rate limiting
app.set('trust proxy', 1);

// Set basic security headers (disabling CSP to not conflict with CryptPad's custom CSP implementation for now)
app.use(helmet({
    contentSecurityPolicy: false,
    crossOriginEmbedderPolicy: false,
    crossOriginOpenerPolicy: false,
    crossOriginResourcePolicy: false,
    xFrameOptions: false, // CryptPad relies on cross-origin iframes
}));
...
const uploadLimiter = rateLimit({
    ...
    limit: 30, // limit each IP to 30 upload requests per windowMs
    message: { error: "Too many uploads from this IP, please try again after 15 minutes" }
});
app.use('/upload-blob', uploadLimiter, Express.json({limit:"500kb"}), (req, res) => {
    if (req.method !== "POST") {
...
const authLimiter = rateLimit({
    ...
    limit: 300, // Allow high RPC volume per minute, block abusive brute forces
...
app.post('/api/auth', authLimiter, function (req, res, next) {
